...
@step_handler("validate_payment_eligibility")
...
@step_handler("process_gateway_refund")
@depends_on(eligibility=("validate_payment_eligibility", PaymentsValidateEligibilityResult))
@inputs(ValidatePaymentEligibilityInput)
def process_gateway(
...
@step_handler("update_payment_records")
@depends_on(
    eligibility=("validate_payment_eligibility", PaymentsValidateEligibilityResult),
    refund_result=("process_gateway_refund", PaymentsProcessGatewayResult),
)
@inputs(ValidatePaymentEligibilityInput)
def update_records(
    eligibility: PaymentsValidateEligibilityResult,
...
@depends_on(
    eligibility=("validate_payment_eligibility", PaymentsValidateEligibilityResult),
    refund_result=("process_gateway_refund", PaymentsProcessGatewayResult),
    records=("update_payment_records", PaymentsUpdateRecordsResult),
)
@inputs(ValidatePaymentEligibilityInput)
def notify_customer(
    eligibility: PaymentsValidateEligibilityResult,
