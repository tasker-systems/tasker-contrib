...
const _MAX_REFUND_AMOUNT = 10_000;
...
export function validateRefundRequest(
	...
	if (
		input.customerId.toLowerCase().includes("vip") ||
		input.customerId.toLowerCase().includes("premium")
	) {
...
export function checkRefundEligibility(
	validationResult: CustomerSuccessValidateRefundResult,
	_refundAmount: number | undefined,
): CustomerSuccessCheckRefundPolicyResult {
	...
	const customerTier = validationResult.customer_tier || "standard";
	const purchaseDateStr = validationResult.original_purchase_date;
...
export function calculateRefundAmount(
	policyResult: CustomerSuccessCheckRefundPolicyResult,
	...
	const requiresApproval = policyResult.requires_approval;
	const customerTier = policyResult.customer_tier;
	const now = new Date().toISOString();
...
export function notifyCustomerSuccess(
	approvalResult: CustomerSuccessApproveRefundResult,
	validationResult: CustomerSuccessValidateRefundResult,
	...
	const paymentId = validationResult?.payment_id;
	if (!paymentId) {
	...
	correlationId =
		correlationId ||
		`cs-corr_${crypto.randomUUID().replace(/-/g, "").substring(0, 12)}`;
...
export function updateCrmRecord(
	delegationResult: CustomerSuccessExecuteRefundResult,
	validationResult: CustomerSuccessValidateRefundResult,
	...
	const delegatedTaskId = delegationResult.delegated_task_id;
	const correlationId = delegationResult.correlation_id;
	...
	return {
		...
		new_status: "resolved",
		resolution_note: `Refund of $${((refundAmount ?? 0) / 100).toFixed(2)} processed successfully. Delegated task ID: ${delegatedTaskId}. Correlation ID: ${correlationId}`,
		updated_at: now,
