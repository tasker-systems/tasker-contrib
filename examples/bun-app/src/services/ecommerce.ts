...
export function processPayment(
	cartResult: EcommerceValidateCartResult,
...
export function createOrder(
	cartResult: EcommerceValidateCartResult,
	paymentResult: EcommerceProcessPaymentResult,
	inventoryResult: EcommerceUpdateInventoryResult,
	...
	const estimatedDeliveryDays = cartResult.free_shipping ? 5 : 3;
	...
	return {
		...
		items: cartResult.validated_items.map((item) => ({
			sku: item.sku,
			name: item.name,
		...
		inventory_reservations: inventoryResult.updated_products.length,
	};
}

export function sendConfirmation(
	orderResult: EcommerceCreateOrderResult,
	cartResult: EcommerceValidateCartResult | undefined,
	customerEmail: string | undefined,
): EcommerceSendConfirmationResult {
	if (!customerEmail) {
		throw new PermanentError("Customer email is required but was not provided");
	...
	const estimatedDelivery = orderResult.estimated_delivery;
