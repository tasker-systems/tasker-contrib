...
export function setupBilling(
	userData: MicroservicesCreateUserResult,
): MicroservicesSetupBillingResult {
	if (!userData) {
	...
	const plan = (userData.plan || "free") as "free" | "pro" | "enterprise";
...
export function sendWelcome(
	userData: MicroservicesCreateUserResult,
	billingData: MicroservicesSetupBillingResult,
	preferencesData: MicroservicesInitPreferencesResult,
): MicroservicesSendWelcomeResult {
...
export function updateStatus(
	...
	billingData: MicroservicesSetupBillingResult,
	preferencesData: MicroservicesInitPreferencesResult,
	welcomeData: MicroservicesSendWelcomeResult,
): MicroservicesUpdateStatusResult {
	const missing: string[] = [];
	if (!userData) missing.push("create_user_account");
	if (!billingData) missing.push("setup_billing_profile");
	...
	if (plan !== "free" && "billing_id" in billingData) {
