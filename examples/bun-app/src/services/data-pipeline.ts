...
export function extractSalesData(
	...
	const records: DataRecord[] = Array.from(
		{ length: Math.min(recordCount, 20) },
		(_, _i) => ({
	...
	return {
		...
		extraction_sources:
			safeSources.filter((s) => s.includes("sales")).length || 1,
		schema_version: "2.1",
	};
}
...
export function extractInventoryData(
	...
	const inventorySnapshot = warehouses.map((wh) => ({
		warehouse_id: wh,
		total_skus:
			Math.floor(skuCount / warehouseCount) + Math.floor(Math.random() * 20),
	...
	const totalUnits = inventorySnapshot.reduce(
		(sum, wh) => sum + wh.total_units,
		0,
	);
...
export function extractCustomerData(
	...
	const activeCustomers = Math.floor(
		totalCustomers * (0.3 + Math.random() * 0.4),
	);
	...
	const regionBreakdown = {
		...
		other: Math.floor(totalCustomers * 0.1),
	};

	// Build simulated customer records for downstream consumption
	const records = Array.from(
		{ length: Math.min(totalCustomers, 10) },
		(_, i) => ({
			customer_id: `CUST-${String(i + 1).padStart(3, "0")}`,
			name: `Customer ${i + 1}`,
			tier: Object.keys(segmentBreakdown)[
				i % Object.keys(segmentBreakdown).length
			],
	...
	const totalLifetimeValue = records.reduce(
		(sum, r) => sum + r.lifetime_value,
		0,
	);
	...
	return {
		...
		avg_lifetime_value:
			Math.round((totalLifetimeValue / records.length) * 100) / 100,
...
export function transformSales(
	extractResults: PipelineExtractSalesResult,
): PipelineTransformSalesResult {
	if (!extractResults) {
		throw new PermanentError("Sales extraction results not found");
	}

	const rawRecords = extractResults.records || [];
	const totalAmount = extractResults.total_amount;

	// Group by category to build product_sales equivalent
	const productSales: Record<
		string,
		{ total_quantity: number; total_revenue: number; order_count: number }
	> = {};
	for (const record of rawRecords) {
		if (!productSales[record.category]) {
			productSales[record.category] = {
				total_quantity: 0,
				total_revenue: 0,
				order_count: 0,
			};
	...
	for (const cat of Object.keys(productSales)) {
		productSales[cat].total_revenue =
			Math.round(productSales[cat].total_revenue * 100) / 100;
	}

	// Calculate time-series aggregation (daily totals)
	const dailySales: Record<
		string,
		{ total_amount: number; order_count: number; avg_order_value: number }
	> = {};
	for (const record of rawRecords) {
	...
	for (const day of Object.keys(dailySales)) {
		dailySales[day].avg_order_value =
			Math.round(
				(dailySales[day].total_amount / dailySales[day].order_count) * 100,
			) / 100;
	}

	const totalRevenue =
		totalAmount || rawRecords.reduce((sum, r) => sum + r.value, 0);
	...
	return {
		...
		avg_transaction_value:
			rawRecords.length > 0
				? Math.round((totalRevenue / rawRecords.length) * 100) / 100
				: 0,
...
export function transformInventory(
	extractResults: PipelineExtractInventoryResult,
): PipelineTransformInventoryResult {
	if (!extractResults) {
	...
	const warehouseSummary: Record<
		string,
		{ total_quantity: number; product_count: number; reorder_alerts: number }
	> = {};
	...
	const productInventory: Record<
		string,
		{ total_quantity: number; warehouse_count: number; needs_reorder: boolean }
	> = {};
	for (const wh of rawRecords) {
	...
	const totalQuantityOnHand = rawRecords.reduce(
		(sum, wh) => sum + wh.total_units,
		0,
	);
...
export function transformCustomers(
	extractResults: PipelineExtractCustomerResult,
): PipelineTransformCustomersResult {
	if (!extractResults) {
	...
	for (const [tier, tierRecords] of tierGroups) {
		...
		tierAnalysis[tier] = {
			...
			avg_lifetime_value:
				Math.round((totalLtv / tierRecords.length) * 100) / 100,
		};
	}
	...
	const totalLifetimeValue = rawRecords.reduce(
		(sum, r) => sum + r.lifetime_value,
		0,
	);
	const avgCustomerValue =
		rawRecords.length > 0 ? totalLifetimeValue / rawRecords.length : 0;
...
export function aggregateMetrics(
	salesData: PipelineTransformSalesResult | undefined,
	inventoryData: PipelineTransformInventoryResult | undefined,
	customerData: PipelineTransformCustomersResult | undefined,
): PipelineAggregateMetricsResult {
	...
	const salesRecordCount = salesData?.record_count || 0;

	const totalInventory = inventoryData?.total_quantity_on_hand || 0;
	const reorderAlerts = inventoryData?.reorder_alerts || 0;

	const totalCustomers = customerData?.record_count || 0;
	const totalLtv = customerData?.total_lifetime_value || 0;

	// Calculate cross-source metrics
	const revenuePerCustomer =
		totalCustomers > 0 ? totalRevenue / totalCustomers : 0;
	const inventoryTurnover =
		totalInventory > 0 ? totalRevenue / totalInventory : 0;
...
export function generateInsights(
	...
	const revenuePerCustomer = metrics.revenue_per_customer || 0;
	...
	// Inventory insights
	const inventoryAlerts = metrics.inventory_reorder_alerts || 0;
	...
	// Customer insights
	const totalLtv = metrics.total_customer_lifetime_value || 0;
