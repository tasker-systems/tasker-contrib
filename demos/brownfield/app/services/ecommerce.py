...
def validate_cart_items(
    ...
    This is the boundary function — raw dicts from the request are validated
    and converted to EcommerceCartItem models here. Downstream functions
    receive typed models only.
    """
    if not cart_items or not isinstance(cart_items, list):
        raise ValidationError("Cart is empty or items field is missing")
    ...
    for idx, item in enumerate(cart_items):
        ...
        if not sku or not name:
            raise ValidationError(f"Item at index {idx} missing sku or name")

        if quantity < 1:
            raise ValidationError(f"Item '{sku}' has invalid quantity: {quantity}")

        if unit_price <= 0:
            raise ValidationError(f"Item '{sku}' has invalid price: {unit_price}")
...
def process_payment(
    ...
    if payment_token in DECLINED_TOKENS:
        raise ValidationError(f"Payment declined for token {payment_token}")

    if payment_token in ERROR_TOKENS:
        raise RuntimeError("Payment gateway returned an error, please retry")
