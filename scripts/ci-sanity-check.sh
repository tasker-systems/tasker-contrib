...
# tasker-contrib repository. Adapted from tasker-core's ci-sanity-check.sh.
#
...
if [ "$SCRIPT_COUNT" -eq 0 ]; then
    ...
    # -S warning: errors + warnings; -s bash: enforce bash dialect;
    # -e SC1091: exclude "not following non-constant source"
...
if [ -d "$WORKFLOW_DIR" ]; then
    ...
    if [ "$WORKFLOW_COUNT" -eq 0 ]; then
        ...
        # Ignore shellcheck info/style-level issues reported by actionlint's
        # embedded shellcheck — typically SC2086:info about quoting ${{ }}
        # expressions, which are safe in GitHub Actions context.
        if xargs actionlint \
            -ignore 'SC[0-9]+:info:' \
            -ignore 'SC[0-9]+:style:' \
            < "$WORKFLOW_LIST"; then
...
# 6a: Parse Makefile.toml files for script references
...
while IFS= read -r makefile; do
    ...
    while IFS= read -r ref; do
        ...
        # Resolve ${SCRIPTS_DIR} to scripts (relative to repo root)
        resolved=$(echo "$ref" | sed 's|\${SCRIPTS_DIR}|scripts|g')

        # If path doesn't start with / or scripts, it's relative to the makefile dir
        case "$resolved" in
            /*|scripts/*) ;;
...
$(grep -E 'file\s*=\s*"' "$makefile" 2>/dev/null || true)
INNEREOF
done <<OUTEREOF
$(find . -name 'Makefile.toml' -not -path '*/target/*' 2>/dev/null | sort)
OUTEREOF

# 6b: Parse workflow files for script references
...
if [ -d "$WORKFLOW_DIR" ]; then
    while IFS= read -r workflow; do
        ...
        # Look for .sh file references that contain a path separator
...
$(grep -oE '[a-zA-Z0-9_./-]+/[a-zA-Z0-9_./-]+\.sh' "$workflow" 2>/dev/null | sort -u || true)
REFEOF
    done <<WFEOF
$(find "$WORKFLOW_DIR" \( -name '*.yml' -o -name '*.yaml' \) -type f 2>/dev/null | sort)
WFEOF
fi
